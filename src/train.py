...
from .models import IBGCN, HGDC, EMOGI, MTGCN, GCN, GAT, GraphSAGE, GIN, ChebNet, ChebNetII, DMGNN, FocalLoss
...
def generate_kde_and_curves(logits, node_names, degree_counts_above, degree_counts_below, labels, train_mask, args):
    """
    Generates KDE plot comparing IBGCN score ranks with KCG interaction ranks, 
    ...
    plt.yticks(fontsize=8)
    plt.xlabel('IBGCN score rank', fontsize=10, labelpad=10)
...
def plot_model_performance(args):
    ...
    models = ["IBGCN", "DMGNN", "Chebnet", "ChebnetII", "HGDC", "EMOGI", "MTGCN", "GCN", "GAT", "GIN"]
    networks = ["CPDB", "STRING", "HIPPIE"]

    # AUPRC values for ONGene and OncoKB for each model (rows: models, cols: networks)
    auroc = [
        [0.9962, 0.9578, 0.9297],  # IBGCN IBGCN & 0.9652 & 0.9783 & 0.9578 & 0.9738 & 0.9297 & 0.9597 \\
        [0.7245, 0.7133, 0.6525],  # HGDC
        [0.6735, 0.8184, 0.6672],  # EMOGI
        [0.6862, 0.7130, 0.6762],  # MTGCN
        [0.7143, 0.6688, 0.6708],  # GCN
        [0.6670, 0.8166, 0.6478],  # GAT
        [0.7134, 0.6166, 0.6571],  # DMGNN
        [0.5836, 0.5173, 0.5844],  # GIN
        [0.8512, 0.8777, 0.7409],  # Chebnet
        [0.6969, 0.6979, 0.6809]   # ChebnetII
    ]

    auprc = [
        [0.9975, 0.9738, 0.9597],  # IBGCN
        [0.8049, 0.7740, 0.7634],  # HGDC
        [0.7230, 0.8737, 0.7960],  # EMOGI
        [0.7712, 0.7878, 0.7785],  # MTGCN
        [0.7729, 0.7681, 0.7675],  # GCN
        [0.7086, 0.8791, 0.7496],  # GAT
        [0.7357, 0.7182, 0.7624],  # DMGNN
        [0.6405, 0.5918, 0.6791],  # GIN
        [0.9019, 0.9159, 0.8443],  # Chebnet
        [0.7727, 0.7898, 0.7882]   # ChebnetII
    ]
