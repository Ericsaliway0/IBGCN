...
class IBGCN_lamda_added(nn.Module):
...
class IBGCN_ori(nn.Module):
    ...
    def forward(self, g, features):
        """
        Forward pass for Fast Adaptive IBGCN.
        ...
        Returns:
        - Output tensor after passing through Fast IBGCN layers
        """
...
class HGDC(nn.Module):
    def __init__(self, in_feats, hidden_feats, out_feats, weights=[0.95, 0.90, 0.15, 0.10]):
        super().__init__()
        self.linear1 = nn.Linear(in_feats, hidden_feats)

        # --- GraphConv layers for multi-branch representation ---
        # k=1
        self.conv_k1_1 = dglnn.GraphConv(hidden_feats, hidden_feats, norm='both')
        self.conv_k1_2 = dglnn.GraphConv(hidden_feats, hidden_feats, norm='both')

        # k=2 (input dim = 2*hidden since concat from k=1)
        self.conv_k2_1 = dglnn.GraphConv(2 * hidden_feats, hidden_feats, norm='both')
        self.conv_k2_2 = dglnn.GraphConv(2 * hidden_feats, hidden_feats, norm='both')

        # k=3 (input dim = 2*hidden since concat from k=2)
        self.conv_k3_1 = dglnn.GraphConv(2 * hidden_feats, hidden_feats, norm='both')
        self.conv_k3_2 = dglnn.GraphConv(2 * hidden_feats, hidden_feats, norm='both')

        # --- Linear layers for final residual mapping ---
        self.linear_r0 = nn.Linear(hidden_feats, out_feats)
        self.linear_r1 = nn.Linear(2 * hidden_feats, out_feats)
        self.linear_r2 = nn.Linear(2 * hidden_feats, out_feats)
        self.linear_r3 = nn.Linear(2 * hidden_feats, out_feats)

        # --- Learnable weights for each residual branch ---
        self.weight_r0 = nn.Parameter(torch.tensor([weights[0]]))
        self.weight_r1 = nn.Parameter(torch.tensor([weights[1]]))
        self.weight_r2 = nn.Parameter(torch.tensor([weights[2]]))
        self.weight_r3 = nn.Parameter(torch.tensor([weights[3]]))

    def forward(self, g, features):
        # R0: basic linear projection
        R0 = F.relu(self.linear1(features))

        # R1: apply k=1 convolutions and concat
        R_k1_1 = F.relu(self.conv_k1_1(g, R0))
        R_k1_2 = F.relu(self.conv_k1_2(g, R0))
        R1 = torch.cat((R_k1_1, R_k1_2), dim=1)

        # R2: apply k=2 convolutions and concat
        R_k2_1 = F.relu(self.conv_k2_1(g, R1))
        R_k2_2 = F.relu(self.conv_k2_2(g, R1))
        R2 = torch.cat((R_k2_1, R_k2_2), dim=1)

        # R3: apply k=3 convolutions and concat
        R_k3_1 = F.relu(self.conv_k3_1(g, R2))
        R_k3_2 = F.relu(self.conv_k3_2(g, R2))
        R3 = torch.cat((R_k3_1, R_k3_2), dim=1)
